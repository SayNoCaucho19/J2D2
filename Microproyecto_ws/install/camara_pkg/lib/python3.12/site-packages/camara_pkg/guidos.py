        ...
        try:
            ...
            if not self.queue.empty():
                ...
                if event[0] == "voltage" and self.voltage_label is not None:
                    ...
                    # ---- NUEVO: actualizar porcentaje de batería ----
                if event[0] == "battery_percent" and self.battery_value is not None:
                    percent = event[1]
                    self.battery_value.configure(text=f"{percent}%")

                    # Colores automáticos
                    if percent > 60:
                        self.battery_value.configure(text_color=OCEAN_COLORS["seaweed"])  # verde
                    elif percent > 30:
                        self.battery_value.configure(text_color=OCEAN_COLORS["gold"])    # amarillo
                    else:
                        self.battery_value.configure(text_color=OCEAN_COLORS["coral"])   # rojo
                if event[0] == "weight":
                    new_weight = event[1]
                    if hasattr(self, "weight_value") and self.weight_value is not None:
