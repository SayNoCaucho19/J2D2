...
class JoystickToSerial(Node):
    def __init__(self):
        ...
        self.declare_parameter('max_speed', 0.15)  # 🔽 REDUCIDO de 0.25 a 0.15
        self.declare_parameter('deadzone', 0.1)
        self.declare_parameter('min_ratio', 0.3)
        
        # 🆕 CALIBRACIÓN DE MOTORES - Ajusta estos valores
        self.declare_parameter('left_motor_factor', 0.85)   # Si va más rápido, baja este valor
        self.declare_parameter('right_motor_factor', 1.0)   # Motor de referencia
        ...
        self.left_motor_factor = self.get_parameter('left_motor_factor').value
        self.right_motor_factor = self.get_parameter('right_motor_factor').value
        ...
        self.get_logger().info(f'🎮 Nodo Teleop iniciado | Velocidad máx: {self.max_speed}')
        self.get_logger().info(f'⚙️  Calibración - Izq: {self.left_motor_factor:.2f} | Der: {self.right_motor_factor:.2f}')
    ...
    def joy_callback(self, msg):
        ...
        self.left_motor = left_raw * self.left_motor_factor
        self.right_motor = right_raw * self.right_motor_factor
