...
class MQTTBridgeNode(Node):
    ...
    def on_connect(self, client, userdata, flags, rc):
        if rc == 0:
            ...
            client.subscribe("aquabot/peso")   # <--- NUEVO
        else:
    ...
    def on_message(self, client, userdata, msg):
        ...
        try:
            ...
            # --- Peso (antes era battery_percentage) ---
            if "battery_percentage" in data:
                peso_valor = float(data["battery_percentage"])   # mismo dato, otro nombre

                msg_ros = Float32()
                msg_ros.data = peso_valor
                self.peso_pub.publish(msg_ros)

                self.get_logger().info(f"⚖️ Peso publicado: {peso_valor}")
